# Operator variables
# ==================
export APP_NAME=selinux-policy-helper-operator

# Container image variables
# =========================
IMAGE_REPO?=quay.io/jaosorior
RUNTIME?=podman
...
export NAMESPACE?=openshift-selinux-policy-helper-operator
...
.PHONY: all
all: build ## Test and Build the selinux-policy-helper-operator
...
build: ## Build the selinux-policy-helper-operator binary
	$(GO) build -o $(TARGET) github.com/JAORMX/selinux-policy-helper-operator/cmd/manager
...
.PHONY: run
run: operator-sdk ## Run the selinux-policy-helper-operator locally
	#WATCH_NAMESPACE=$(NAMESPACE) 
	KUBERNETES_CONFIG=$(KUBECONFIG) \
	OPERATOR_NAME=selinux-policy-helper-operator \
...
namespace:
	...
	@kubectl create -f deploy/ns.yaml || true
